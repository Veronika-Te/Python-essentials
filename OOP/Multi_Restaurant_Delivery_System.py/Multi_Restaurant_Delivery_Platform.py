...
class Appetizer(MenuItem):
    ...
    def __init__(self,name:str, price: Decimal, glutenfree: bool, ingredient: List[str])->None:
...
class Users(ABC):
    __slots__ = ('__name','__email','__phonenumber')
    def __init__(self, name: str, email: str, phonenumber: int):
        self.name = name
        self.email = email
...
class Customer(Users):                                                                                            
    __slots__= ('__order_history', '__reviews') 
    
    def __init__(self, name: str, email: str, phonenumber: int)->None:
    ...
    def get_user_info(self):
        return f"Customer name: {self.name}, Email: {self.email}, Phone: {self.phonenumber}"
...
class Driver(Users):
  __slots__ = ('__assigned_orders')
  def __init__(self, name: str,  email:str, phonenumber: int):
    super().__init__(name, email, phonenumber)
    self.__assigned_orders=[]
  
  def get_user_info(self):
    return f"Driver name: {self.name}, Email: {self.email}, Phone: {self.phonenumber}"
  ...
  def deliver_order(self, order: Order):
    if order in self.__assigned_orders:
      ...
      print("This order is not assigned to the driver.")
  ...
  def __str__(self):
    return f"Driver name: {self.name}, Contact info: {self.contact_info()}, Assigned orders: {len(self.__assigned_orders)}"
...
def main():
    ...
    try:
        ...
        phone=193337     
        driver1=Driver(name, email, phone)
        print(driver1," \n")
        driver1.accept_order(dine)
